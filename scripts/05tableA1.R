###--- Generate Table A1 in the paper
...
###--- Load ego and alter ids
egos_file <- readRDS("data/ids/ego_to_int.RDS")
...
ideo_egos <-  
  ...
  mutate(ideo_bi = ifelse(ideology_new > wsj_ideo, "Conservative", "Liberal")) %>% 
  left_join(egos_file %>% rename(user = user_id))


ideo_alters <-  
  readRDS("data/ideology/alter_ideology.RDS") %>% 
  mutate(ideo_bi = ifelse(ideology_new > wsj_ideo, "Conservative", "Liberal")) %>% 
  left_join(alters_file %>% rename(user = user_id))
...
(r3 <-  
    ...
    drop_na() %>% 
    mutate(cat = "Average number of ties",
...
###--- Exposure Volume 
data <-  readRDS("data/main/ego_alter_domain_tabs_binary_wsj_cutpoint.RDS")
...
r6 = 
  tibble(ideo = c("Conservative", "Liberal"), 
...
table1 <- xtable(tbl)
print(table1, type = "html", file = "output/tableA1.html", include.rownames = FALSE)
